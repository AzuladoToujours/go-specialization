...
Each line of the text file has a first name and a last name, in that order, separated by a single space
on the line.
...
After reading all lines from the file, your program should iterate through your slice of
...
func main() {
	people := make([]Name, 0, 20)
	var path string
	fmt.Println("File's path:")
	fmt.Scan(&path)
	file, _ := os.Open(path)
	scanner := bufio.NewScanner(file)
	for scanner.Scan() {
		names := strings.Split(scanner.Text(), " ")
		name := Name{firstName: names[0], lastName: names[1]}
		people = append(people, name)
	}
	fmt.Println("Names: ",people)
	file.Close()
}
